...
2.  **Open the project in Unity**:
    ...
    - Open the project.
3.  **Create API Key Configuration (TODO: implement a more secure method)**:
...
4.  **Configure Services**:
    ...
    - Select each configuration file and fill in the required fields in the Inspector, such as model names, regions, and voice IDs. Leave API keys blank as they are loaded from `keys.json`
...
2.  **Create a configuration ScriptableObject**: Create a new class that inherits from one of the generic service configs (`GenericChatServiceConfig`, `GenericSTTServiceConfig`, or `GenericTTSServiceConfig`) to hold the configuration for your new service.
3.  **Define a constructor for your ScriptableObject**: Define a constructor for your config that assigns `serviceManagerType = typeof(YourServiceClass)` and `apiKeyJSONString = "YOUR_SERVICE_API_KEY_NAME"` where `YOUR_SERVICE_API_KEY_NAME` is the string in `keys.json` that designates the api key for the service.
